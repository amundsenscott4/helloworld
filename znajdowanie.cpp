...
int szukaj_liczby(int* a, int v, int start, int finish){
    ...
    int srodek = (start+finish)/2;
    if (a[srodek] == v){
        return srodek;
    }
    if (a[srodek]<v) {
        return szukaj_liczby(a, v, srodek+1, finish);
    }
    else {
        return szukaj_liczby(a, v, start, srodek-1);
    }
}
...
int main() {
    ...
    //}
    cout << "Indeks szukanej liczby: " << szukaj_liczby(test[1], 10, 0, 10);
}